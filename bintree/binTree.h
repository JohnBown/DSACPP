...
template <typename T> BinNodePosi(T) BinTree<T>::insertAsRoot(T const& e) { //将e当作根节点插入空的二叉树
    _size        = 1;
...
template <typename T> static int removeAt(BinNodePosi(T) x) { //删除x处的节点及其后代, 返回被删节点数
    if (!x) return 0;                                         //递归基: 空树
    int n = 1 + removeAt(x->lc) + removeAt(x->rc);            //释放左, 右子树
    if (x->data) delete x->data;                              // release(x->data)
    if (x) delete x;                                          // release(x)
    return n;                                                 //返回被删除节点总数
}

template <typename T> int BinTree<T>::remove(BinNodePosi(T) x) { //删除x处的节点及其后代, 返回被删节点数
...
template <typename T> BinTree<T>* BinTree<T>::secede(BinNodePosi(T) x) { //将子树x从当前树中摘除, 并返回该树
    FromParentTo(*x) = nullptr;                                          //切断来自父节点的指针
    updateHeightAbove(x->parent);                                        //更新祖先高度
    BinTree<T>* S = new BinTree<T>;                                      //以新树x为根
    S->_root      = x;
    x->parent     = nullptr;
    S->_size      = x->size(); //更新规模
    _size -= S->_size;
