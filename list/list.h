...
ListNodePosi(T) List<T>::search(T const &e, int n, ListNodePosi(T) p) const {
    while (0 <= n--) {                       // p的n个前驱, 从右向左逐个比较
        if ((p = p->pred)->data <= e) break; //直至命中、数值越界或范围哦越界
    }
    return p; //返回查找终止的位置
}
...
ListNodePosi(T) List<T>::selectMax(ListNodePosi(T) p, int n) { //从起始于位置p的n个元素中选出最大者
    ...
    for (ListNodePosi(T) cur = p; 1 < n; n--) { //从首节点p出发, 将后续节点逐一与max比较
        cur = cur->succ;
        if (max->data <= cur->data) max = cur; //更新最大元素位置记录
    }
    return max; //返回最大节点位置
}
...
void List<T>::merge(ListNodePosi(T) & p, int n, List<T> &L, ListNodePosi(T) & q, int m) {
    ...
    while (0 < m) {                            //在q尚未移出区间之前
        if ((0 < n) && (p->data <= q->data)) { // p也未移出区间, 且L[p] <= L[q]
            if (q == (p = p->succ)) break;     // p归入合并列表, 并替换为其直接后继
            n--;
        } else {                                       //若已超出右界或L[q] < L[p]
...
template <typename T> int List<T>::uniquify() { //成批剔除重复元素, 效率更高
    ...
    //反复考察紧邻节点对(p, q),若互异, 则转向下一区段,否则(雷同), 删除后者
    while (trailer != (q = p->succ)) { (p->data != q->data) ? p = q : remove(q); }
    return oldSize - _size; //列表规模变化量, 既被删除元素总数
}
...
template <typename T> void List<T>::reverse() { //前后倒置. 共计三种实现方式, 采用了较复杂的其一
    ...
    // 3.自前向后, 依次颠倒各节点的后继指针
    // 4.单独设置头节点的后继指针
    for (p = header, q = p->succ; p != trailer; p = q, q = p->succ) { p->pred = q; } // 1
    trailer->pred = nullptr;                                                         // 2
    for (p = header, q = p->pred; p != trailer; p = q, q = p->pred) { q->succ = p; } // 3
    header->succ = nullptr;                                                          // 4
